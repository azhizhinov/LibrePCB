...
KeyboardShortcutsModel::KeyboardShortcutsModel(QObject* parent) noexcept
  : QAbstractItemModel(parent), mOverrides() {
}
...
int KeyboardShortcutsModel::rowCount(const QModelIndex& parent) const noexcept {
  if (!parent.isValid()) {
    return EditorCommandSet::instance().getCategories().count();
  } else if (const EditorCommandCategory* category =
                 categoryFromIndex(parent)) {
    return EditorCommandSet::instance().getCommands(category).count();
  } else {
    return 0;
  }
}
...
QModelIndex KeyboardShortcutsModel::index(int row, int column,
  ...
  } else if (EditorCommandCategory* category = categoryFromIndex(parent)) {
...
QModelIndex KeyboardShortcutsModel::parent(const QModelIndex& index) const
    noexcept {
  if (index.isValid()) {
    if (EditorCommandCategory* category =
            static_cast<EditorCommandCategory*>(index.internalPointer())) {
      int index =
          EditorCommandSet::instance().getCategories().indexOf(category);
...
QVariant KeyboardShortcutsModel::data(const QModelIndex& index, int role) const
    noexcept {
  if (const EditorCommandCategory* category = categoryFromIndex(index)) {
    if (index.column() == 0) {
      switch (role) {
        case Qt::DisplayRole: {
    ...
    switch (index.column()) {
      case 0: {
        switch (role) {
          case Qt::DisplayRole:
            return QString(command->getText()).remove("...");
...
EditorCommandCategory* KeyboardShortcutsModel::categoryFromIndex(
    const QModelIndex& index) const noexcept {
  if ((index.model() == this) && (index.isValid()) &&
      (!index.internalPointer())) {
    return EditorCommandSet::instance().getCategories().value(index.row());
  } else {
    return nullptr;
  }
}

EditorCommand* KeyboardShortcutsModel::commandFromIndex(
    const QModelIndex& index) const noexcept {
  if ((index.model() == this) && (index.isValid()) &&
      (index.internalPointer())) {
    EditorCommandCategory* category =
        static_cast<EditorCommandCategory*>(index.internalPointer());
    return EditorCommandSet::instance().getCommands(category).value(
        index.row());
  } else {
    return nullptr;
  }

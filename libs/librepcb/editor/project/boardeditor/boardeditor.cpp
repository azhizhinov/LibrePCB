...
void BoardEditor::createActions() noexcept {
  ...
  mActionCloseProject.reset(cmd.closeProject.createAction(
  ...
  mActionCopy.reset(
      cmd.copy.createAction(this, mFsm.data(), &BoardEditorFsm::processCopy));
  mActionCut.reset(
      cmd.cut.createAction(this, mFsm.data(), &BoardEditorFsm::processCut));
  ...
  mActionFlipHorizontal.reset(cmd.flipHorizontal.createAction(
      this, mFsm.data(), &BoardEditorFsm::processFlipHorizontal));
  mActionFlipVertical.reset(cmd.flipVertical.createAction(
      this, mFsm.data(), &BoardEditorFsm::processFlipVertical));
  ...
  mActionNewBoard.reset(
      cmd.newBoard.createAction(this, this, &BoardEditor::newBoard));
  mActionCopyBoard.reset(
      cmd.copyBoard.createAction(this, this, &BoardEditor::copyBoard));
  mActionRemoveBoard.reset(
      cmd.removeBoard.createAction(this, this, &BoardEditor::removeBoard));
  mActionOrderPcb.reset(cmd.orderPcb.createAction(this, this, [this]() {
    mProjectEditor.execOrderPcbDialog(nullptr, this);
  }));
  mActionPaste.reset(
      cmd.paste.createAction(this, mFsm.data(), &BoardEditorFsm::processPaste));
  ...
  mActionQuit.reset(cmd.quit.createAction(this, this, &BoardEditor::close));
  ...
  mActionRotateCcw.reset(cmd.rotateCcw.createAction(
      this, mFsm.data(), &BoardEditorFsm::processRotateCcw));
  mActionRotateCw.reset(cmd.rotateCw.createAction(
      this, mFsm.data(), &BoardEditorFsm::processRotateCw));
  mActionSaveProject.reset(cmd.saveProject.createAction(
  ...
  mActionUpdateLibrary.reset(
      cmd.updateProjectLibrary.createAction(this, this, [this]() {
  ...
  mActionZoomFit.reset(cmd.zoomFit.createAction(this, mUi->graphicsView,
  ...
  mActionRebuildPlanes.reset(
      cmd.rebuildPlanes.createAction(this, this, [this]() {
        if (Board* board = getActiveBoard()) {
          board->rebuildAllPlanes();
          board->forceAirWiresRebuild();
        }
      }));
  mActionShowPlanes.reset(cmd.showPlanes.createAction(this, this, [this]() {
    if (Board* board = getActiveBoard()) {
  ...
  mActionHidePlanes.reset(cmd.hidePlanes.createAction(this, this, [this]() {
    if (Board* board = getActiveBoard()) {
      foreach (BI_Plane* plane, board->getPlanes()) {
        // No undo command needed since it is not saved.
...
void BoardEditor::createMenus() noexcept {
  ...
  mb.addSection(tr("Docks"));
  mb.addAction(mDockUnplacedComponents->toggleViewAction());
  mb.addAction(mDockLayers->toggleViewAction());
  mb.addAction(mDockErc->toggleViewAction());
  mb.addAction(mDockDrc->toggleViewAction());
